...
class XlsxToPdfConverter {
  static Future<File> convert({
    ...
    int addedPages = 0;
    final ext = p.extension(inputXlsxPath).toLowerCase();

    if (ext == '.csv') {
      onProgress?.call('Reading CSV file...');
      final content = await File(inputXlsxPath).readAsString();
      final rows = const CsvToListConverter().convert(content);
      final List<List<String>> tableData = rows.map((r) => r.map((c) => _cleanText(c.toString())).toList()).toList();
      ...
      pdf.addPage(
        pw.Page(
          ...
          build: (context) => pw.Center(
            child: pw.Text('Empty Spreadsheet', style: const pw.TextStyle(fontSize: 14, color: PdfColors.grey600)),
          ),
        ),
      );
    }
